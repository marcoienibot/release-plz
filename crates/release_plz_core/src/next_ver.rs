...
fn process_git_only_package(
    ...
    if reconstructed_workspaces.contains_key(&release_commit) {
        debug!(
        ...
        reconstructed_workspaces.insert(release_commit.clone(), worktree);
    }
    let worktree = &reconstructed_workspaces[&release_commit];
