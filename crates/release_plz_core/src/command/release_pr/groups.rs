...
fn release_groups(input: &UpdateRequest) -> anyhow::Result<Vec<Vec<String>>> {
    ...
    for a in 0..packages.len() {
        for b in a + 1..packages.len() {
            ...
            let linked = input.workspace_changelog_path().is_some()
                || dependency_paths[a].contains(&package_paths[b])
...
mod tests {
    ...
    async fn atomic_groups_open_isolated_prs_and_reuse_them_on_rerun() {
        exercise_atomic_prs(false).await;
    }

    #[tokio::test]
    async fn workspace_rollup_opens_one_atomic_pr_and_reuses_it() {
        exercise_atomic_prs(true).await;
    }

    async fn exercise_atomic_prs(rollup: bool) {
        ...
        let request = if rollup {
            request.with_workspace_changelog("docs/WORKSPACE.md".into())
        } else {
            request
        };
        ...
        let expected_prs = if rollup { 1 } else { 2 };
        assert_eq!(first.len(), expected_prs);
        for pr in &first {
            assert_eq!(pr.releases.len(), if rollup { 2 } else { 1 });
            if rollup {
                let content = repo
                    .git(&[
                        "--git-dir",
                        ...
                        "show",
                        &format!("{}:docs/WORKSPACE.md", pr.head_branch),
                    ])
                    .unwrap();
                assert!(content.contains("### a 0.1.0"));
                assert!(content.contains("### a-b 0.1.0"));
                assert!(content.contains("../a/CHANGELOG.md"));
                for name in ["a", "a-b"] {
                    assert!(
                        repo.git(&[
                            "--git-dir",
                            remote.path().to_str().unwrap(),
                            "show",
                            &format!("{}:{name}/CHANGELOG.md", pr.head_branch)
                        ])
                        .unwrap()
                        .contains("0.1.0")
                    );
        ...
        assert_eq!(state.lock().unwrap().len(), expected_prs);
        let selected =
        ...
        assert_eq!(selected_prs[0].releases.len(), if rollup { 2 } else { 1 });
        assert!(
            selected_prs[0]
                .releases
                .iter()
                .any(|release| release.package_name == "a")
        );
        assert_eq!(state.lock().unwrap().len(), expected_prs);
