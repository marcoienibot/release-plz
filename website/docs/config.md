...
- [`[workspace]`](#the-workspace-section) — Configuration applied to all packages by default.
  ...
  - [`version_prefix_pattern`](#the-version_prefix_pattern-field) — Parse custom changelog heading prefixes.
...
#### The `version_prefix_pattern` field

Use this option when your changelog release headings contain a package name or
another prefix that the default parser does not recognize. It affects parsing
for release PR descriptions, GitHub release notes, and `set-version`; it does
not change the heading format generated by your git-cliff template.

For headings such as `## [my-crate - 1.2.3] - 2026-01-01`:

```toml
[workspace]
version_prefix_pattern = 'my-crate\s*-\s*'

[[package]]
name = "other-crate"
version_prefix_pattern = 'other-crate\s*-\s*'
```

Package settings override the workspace pattern. If omitted, the default
[parse-changelog prefix format](https://docs.rs/parse-changelog/latest/parse_changelog/struct.Parser.html#method.prefix_format)
is used. Use TOML literal strings (single quotes) to preserve regex backslashes.
The pattern matches the prefix before the version, not the Markdown heading markers.
...
#### The `version_prefix_pattern` field (`package` section)

Overrides [`workspace.version_prefix_pattern`](#the-version_prefix_pattern-field)
when parsing this package's changelog.
